...
if __name__ == "__main__":
    ...
    try:
        if not wait_for_coordinates(state):
            sys.exit(1)
        TARGET_ALT_M = 20.0
        print("Очищаем старую миссию и загружаем новую из одной точки (без остановки мониторинга)...")
        clear_mission(master)
        mission_items = create_single_waypoint_mission_from_state(state, TARGET_ALT_M)
        upload_mission(master, mission_items, state)  # Теперь с state, без остановки
